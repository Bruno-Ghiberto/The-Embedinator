...
The ResearchGraph is Layer 2 of the three-layer LangGraph agent architecture. One ResearchGraph instance is spawned per sub-question via LangGraph `Send()` from the ConversationGraph's `route_fan_out()` edge function (called via `route_after_rewrite()` in `backend/agent/edges.py`). The Send() target node is `"research"`. Multiple instances run concurrently. Each instance manages its own tool-call loop with deduplication, safety limits, context compression, and confidence tracking.
...
8. **Answer Collection**: Package the final answer with citations and confidence score for return to ConversationGraph. Note: `ResearchState.confidence_score` is `float` (0.0–1.0) but `SubAnswer.confidence_score` is `int` (0–100). The `collect_answer` node must convert via `int(confidence_score * 100)`.
9. **Confidence Computation**: The `confidence_score` must be computed from measurable retrieval signals (rerank scores, chunk count, coverage), NOT from LLM self-assessment. See `compute_confidence()` in architecture-design.md.

### Non-Functional Requirements

1. Token counting must use `count_tokens_approximately` from `langchain_core.messages.utils` (zero new dependencies, already used in spec-02). Do NOT add `tiktoken` as a dependency.
...
|------|-------------|-------|--------|-------------|
| `search_child_chunks` | Hybrid dense+BM25 search in Qdrant on child chunk collection | `query: str, collection: str, top_k: int, filters: dict | None` | `list[RetrievedChunk]` | Qdrant search + cross-encoder rerank |
...
- **Spec 02 (ConversationGraph)**: Spawns ResearchGraph via `Send("research", payload)` from `route_fan_out()` edge function -- ResearchGraph must conform to the `ResearchState` payload populated in `backend/agent/edges.py:87-101`
- **Spec 04 (MetaReasoningGraph)**: ResearchGraph routes to MetaReasoningGraph on low confidence + budget exhaustion
- **Libraries**: `langgraph >= 1.0.10`, `langchain >= 1.2.10`, `qdrant-client >= 1.17.0`, `sentence-transformers >= 5.2.3`
...
async def orchestrator(
    ...
    LLM is resolved from state["llm_model"] via provider registry.
...
async def collect_answer(
    ...
    Confidence is computed from measurable retrieval signals, NOT LLM self-assessment.
    Converts float (0.0-1.0) to int (0-100) for SubAnswer.
    Reads: state["sub_question"], state["retrieved_chunks"]
...
async def compress_context(
    ...
    Uses count_tokens_approximately from langchain_core.messages.utils.
