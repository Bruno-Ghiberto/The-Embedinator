...
The ConversationGraph is Layer 1 of the three-layer LangGraph agent architecture. It is the outermost graph and the entry point for every chat request. It manages the full conversation lifecycle: session initialization, intent classification, query decomposition into sub-questions, parallel dispatch to ResearchGraph instances via LangGraph `Send()`, answer aggregation, grounded answer verification (Phase 2), citation validation (Phase 2), and final response formatting for NDJSON streaming.

**File**: `backend/agent/conversation_graph.py`

The ConversationGraph is session-scoped (no max duration). It receives a chat message from the FastAPI chat endpoint, processes it through a series of nodes, and returns a formatted response with inline citations, confidence indicators, and groundedness annotations (Phase 2).

> **Phase Note**: This spec covers the full ConversationGraph design. Requirements 1–6, 9–10 are immediate (replace the current direct retrieval pipeline in `chat.py`). Requirements 7 (GAV) and 8 (citation-chunk alignment) are Phase 2 features that build on top of the ConversationGraph foundation. Nodes for Phase 2 features should be defined with stub implementations that pass through without modification.
...
7. **Grounded Answer Verification (GAV)** *(Phase 2)*: Run NLI-based claim-by-claim verification of the draft answer against retrieved chunks. Mark claims as SUPPORTED, UNSUPPORTED, or CONTRADICTED. If >50% of claims are unsupported, flag the answer. Phase 1 stub: skip verification, pass through `groundedness_result = None`.
8. **Citation-Chunk Alignment Validation** *(Phase 2)*: Use cross-encoder to verify each inline citation points to a chunk that actually supports the claim. Remap or remove invalid citations. Phase 1 stub: pass citations through unvalidated.
9. **Response Formatting**: Apply citation annotations `[1]`, `[2]`, etc. Add confidence indicators. When GAV is active (Phase 2), annotate unverified claims with `[unverified]` and remove contradicted claims. Format for NDJSON delivery per ADR-007.
...
| `format_response` | Apply citation annotations, confidence indicator, format for NDJSON streaming | `final_response`, `citations`, `groundedness_result`, `confidence_score` | `final_response` (formatted) | None |
...
class ConversationState(TypedDict):
    ...
    intent: str  # "rag_query" | "collection_mgmt" | "ambiguous" — set by classify_intent
    ...
    confidence_score: int  # 0–100 scale (user-facing), matches constitution Principle IV
    iteration_count: int
```
...
- **Spec 01 (Vision)**: State schemas (`backend/agent/state.py`), Pydantic models (`backend/agent/schemas.py`), config, errors, SQLite DB, Qdrant client. The existing `ConversationState` TypedDict MUST be extended (add `intent` field) rather than replaced. The existing `Citation`, `SubAnswer`, `RetrievedChunk`, `QueryAnalysis`, `ClaimVerification`, and `GroundednessResult` schemas are already implemented and MUST be reused.
- **Spec 01 prompts**: The existing `backend/agent/prompts.py` contains Phase 1 prompt constants (`SYSTEM_PROMPT`, `QUERY_ANALYSIS_PROMPT`, `ANSWER_SYNTHESIS_PROMPT`, `GROUNDEDNESS_CHECK_PROMPT`). This spec adds more granular prompt constants (`CLASSIFY_INTENT_SYSTEM/USER`, `REWRITE_QUERY_SYSTEM/USER`, `VERIFY_GROUNDEDNESS_SYSTEM`, `FORMAT_RESPONSE_SYSTEM`). New constants MUST be added to the same file; existing constants MAY be preserved for backward compatibility or removed if fully superseded.
...
- **Services**: Ollama or cloud LLM provider for classify_intent, rewrite_query, verify_groundedness (Phase 2); Cross-encoder model for validate_citations (Phase 2)
...
### Citation Validation Implementation *(Phase 2)*

> **Schema note**: The existing `Citation` model (from spec-01 `schemas.py`)
> has fields: `passage_id`, `document_id`, `document_name`, `start_offset`,
> `end_offset`, `text`, `relevance_score`. The validation below uses
> `citation.text` (the passage excerpt) as the "claim text" and cross-checks
> it against all retrieved chunks from sub-answers.
...
async def validate_citations(
    ...
    """Verify each citation points to a chunk that supports the claim. (Phase 2)"""
    corrected_citations: list[Citation] = []

    # Flatten all retrieved chunks from sub-answers
    all_chunks = [sa.chunks for sa in state["sub_answers"]]
    flat_chunks = [c for sublist in all_chunks for c in sublist]

    for citation in state["citations"]:
        # Use the citation's passage text as the claim to validate
        claim_text = citation.text

        # Find the matching chunk by passage_id
        matching_chunk = next(
            (c for c in flat_chunks if c.chunk_id == citation.passage_id),
            None,
        )

        if matching_chunk is None:
            continue  # Citation references a chunk we don't have

        score = reranker.predict([(claim_text, matching_chunk.text)])[0]
        ...
        if score >= CITATION_ALIGNMENT_THRESHOLD:
            ...
            if best_score >= CITATION_ALIGNMENT_THRESHOLD:
                best_chunk = flat_chunks[best_idx]
                # Remap citation to point to the better-matching chunk
                citation = citation.model_copy(update={
                    "passage_id": best_chunk.chunk_id,
                    "document_name": best_chunk.source_file,
                    "text": best_chunk.text[:200],
...
### NDJSON Streaming Format (ADR-007 / Constitution Principle VI)
...
{"type": "chunk", "text": " that retrieval accuracy..."}
{"type": "metadata", "trace_id": "7f3a1c2d-...", "confidence": 87, "citations": [{"passage_id": "p-001", "document_id": "doc-abc", "document_name": "report.pdf", "text": "...", "relevance_score": 0.94, "source_removed": false}], "latency_ms": 1240}
```

> **Note**: The architecture-design.md sequence diagram (lines 420-425) shows an
> older SSE format (`data: {"type": "token", ...}`). That format is superseded
> by ADR-007. Use the NDJSON format above.
...
9. Formatted response streamed via NDJSON (`application/x-ndjson`) to Next.js to Browser
