...
class TestCreateCollection:
    @pytest.mark.asyncio
    async def test_create_collection_calls_qdrant(self, storage: QdrantStorage, client: AsyncMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_create_collection_dense_and_sparse_config(self, storage: QdrantStorage, client: AsyncMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_create_collection_resets_circuit_on_success(self, storage: QdrantStorage, client: AsyncMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_create_collection_records_failure_on_error(self, storage: QdrantStorage, client: AsyncMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_create_collection_raises_circuit_open(self, storage: QdrantStorage, client: AsyncMock) -> None:
...
class TestCollectionExists:
    @pytest.mark.asyncio
    async def test_collection_exists_true(self, storage: QdrantStorage, client: AsyncMock) -> None:
        client.collection_exists = AsyncMock(return_value=True)
        assert await storage.collection_exists("col") is True

    @pytest.mark.asyncio
    async def test_collection_exists_false(self, storage: QdrantStorage, client: AsyncMock) -> None:
        client.collection_exists = AsyncMock(return_value=False)
        assert await storage.collection_exists("missing") is False

    @pytest.mark.asyncio
    async def test_collection_exists_propagates_exception(self, storage: QdrantStorage, client: AsyncMock) -> None:
...
class TestDeleteCollection:
    @pytest.mark.asyncio
    async def test_delete_collection(self, storage: QdrantStorage, client: AsyncMock) -> None:
...
class TestGetCollectionInfo:
    @pytest.mark.asyncio
    async def test_get_collection_info_returns_dict(self, storage: QdrantStorage, client: AsyncMock) -> None:
        mock_info = MagicMock()
...
class TestBatchUpsert:
    ...
    @pytest.mark.asyncio
    async def test_batch_upsert_returns_count(self, storage: QdrantStorage, client: AsyncMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_batch_upsert_empty_list_returns_zero(self, storage: QdrantStorage, client: AsyncMock) -> None:
        count = await storage.batch_upsert("col", [])
        assert count == 0
        client.upsert.assert_not_called()

    @pytest.mark.asyncio
    async def test_batch_upsert_calls_qdrant_upsert(self, storage: QdrantStorage, client: AsyncMock) -> None:
        client.upsert = AsyncMock()
    ...
    @pytest.mark.asyncio
    async def test_batch_upsert_idempotent_same_id(self, storage: QdrantStorage, client: AsyncMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_batch_upsert_timeout_fails_entire_batch(self, storage: QdrantStorage, client: AsyncMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_batch_upsert_with_sparse_vector(self, storage: QdrantStorage, client: AsyncMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_batch_upsert_without_sparse_vector(self, storage: QdrantStorage, client: AsyncMock) -> None:
        """Points without sparse vectors only include the dense vector."""
        client.upsert = AsyncMock()
        point = QdrantPoint(id=1, vector=DENSE_VECTOR, sparse_vector=None, payload=VALID_PAYLOAD.copy())
        await storage.batch_upsert("col", [point])
        call_kwargs = client.upsert.call_args.kwargs
        upserted = call_kwargs["points"][0]
...
class TestPayloadValidation:
    ...
    @pytest.mark.asyncio
    async def test_batch_upsert_rejects_invalid_payload(self, storage: QdrantStorage, client: AsyncMock) -> None:
        bad_payload = VALID_PAYLOAD.copy()
        del bad_payload["chunk_hash"]
        point = QdrantPoint(id=1, vector=DENSE_VECTOR, sparse_vector=None, payload=bad_payload)
...
class TestSearchHybrid:
    @pytest.mark.asyncio
    async def test_search_dense_only(self, storage: QdrantStorage, client: AsyncMock) -> None:
        """With no sparse vector, only dense search is executed."""
        client.query_points = AsyncMock(return_value=make_query_response([make_scored_point(1, 0.9, VALID_PAYLOAD)]))
    ...
    @pytest.mark.asyncio
    async def test_search_with_sparse_vector(self, storage: QdrantStorage, client: AsyncMock) -> None:
        """With sparse vector, two searches are performed (dense + sparse)."""
        client.query_points = AsyncMock(return_value=make_query_response([make_scored_point(1, 0.8, VALID_PAYLOAD)]))
        results = await storage.search_hybrid("col", DENSE_VECTOR, SPARSE_VEC, top_k=5)
        # query_points called twice: dense + sparse
        assert client.query_points.call_count == 2
        assert len(results) >= 1

    @pytest.mark.asyncio
    async def test_search_returns_top_k(self, storage: QdrantStorage, client: AsyncMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_search_applies_score_threshold(self, storage: QdrantStorage, client: AsyncMock) -> None:
        ...
        client.query_points = AsyncMock(return_value=make_query_response(hits))
        results = await storage.search_hybrid("col", DENSE_VECTOR, None, top_k=10, score_threshold=0.5)
        # Only the 0.9 score should pass threshold (dense_weight=0.6 * 0.9 = 0.54)
        for r in results:
            assert r.score >= 0.5

    @pytest.mark.asyncio
    async def test_search_weighted_fusion(self, storage: QdrantStorage, client: AsyncMock) -> None:
        ...
        results = await storage.search_hybrid("col", DENSE_VECTOR, SPARSE_VEC, dense_weight=0.6, sparse_weight=0.4)
        assert len(results) == 1
        expected = 0.6 * 1.0 + 0.4 * 0.5
        assert abs(results[0].score - expected) < 1e-6

    @pytest.mark.asyncio
    async def test_search_results_sorted_descending(self, storage: QdrantStorage, client: AsyncMock) -> None:
        hits = [
    ...
    @pytest.mark.asyncio
    async def test_search_returns_search_result_objects(self, storage: QdrantStorage, client: AsyncMock) -> None:
        client.query_points = AsyncMock(return_value=make_query_response([make_scored_point(7, 0.75, VALID_PAYLOAD)]))
        results = await storage.search_hybrid("col", DENSE_VECTOR, None, top_k=5)
        assert all(isinstance(r, SearchResult) for r in results)

    @pytest.mark.asyncio
    async def test_search_propagates_qdrant_error(self, storage: QdrantStorage, client: AsyncMock) -> None:
...
class TestDeletePoints:
    @pytest.mark.asyncio
    async def test_delete_points_by_ids(self, storage: QdrantStorage, client: AsyncMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_delete_points_empty_list_returns_zero(self, storage: QdrantStorage, client: AsyncMock) -> None:
        count = await storage.delete_points("col", [])
        assert count == 0
        client.delete.assert_not_called()

    @pytest.mark.asyncio
    async def test_delete_points_nonexistent_ids_graceful(self, storage: QdrantStorage, client: AsyncMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_delete_points_by_filter(self, storage: QdrantStorage, client: AsyncMock) -> None:
        client.delete = AsyncMock()
    ...
    @pytest.mark.asyncio
    async def test_delete_points_propagates_error(self, storage: QdrantStorage, client: AsyncMock) -> None:
...
class TestPointRetrieval:
    @pytest.mark.asyncio
    async def test_get_point_returns_dict(self, storage: QdrantStorage, client: AsyncMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_get_point_not_found_returns_none(self, storage: QdrantStorage, client: AsyncMock) -> None:
        client.retrieve = AsyncMock(return_value=[])
        result = await storage.get_point("col", 9999)
        assert result is None

    @pytest.mark.asyncio
    async def test_get_points_by_ids_batch(self, storage: QdrantStorage, client: AsyncMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_get_points_by_ids_empty(self, storage: QdrantStorage, client: AsyncMock) -> None:
        results = await storage.get_points_by_ids("col", [])
        assert results == []
        client.retrieve.assert_not_called()

    @pytest.mark.asyncio
    async def test_scroll_points_returns_tuple(self, storage: QdrantStorage, client: AsyncMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_scroll_points_end_of_collection(self, storage: QdrantStorage, client: AsyncMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_scroll_points_pagination(self, storage: QdrantStorage, client: AsyncMock) -> None:
        first_page = [make_record(i) for i in range(5)]
        second_page = [make_record(i + 5) for i in range(3)]

        async def scroll_side_effect(*args: Any, **kwargs: Any) -> tuple[list, int | None]:
...
class TestHealthCheck:
    @pytest.mark.asyncio
    async def test_health_check_returns_true(self, storage: QdrantStorage, client: AsyncMock) -> None:
        client.get_collections = AsyncMock(return_value=MagicMock(collections=[]))
        assert await storage.health_check() is True

    @pytest.mark.asyncio
    async def test_health_check_returns_false_on_error(self, storage: QdrantStorage, client: AsyncMock) -> None:
        client.get_collections = AsyncMock(side_effect=ConnectionError("refused"))
        assert await storage.health_check() is False
        assert storage._failure_count > 0

    @pytest.mark.asyncio
    async def test_health_check_resets_circuit_on_success(self, storage: QdrantStorage, client: AsyncMock) -> None:
...
class TestCircuitBreaker:
    @pytest.mark.asyncio
    async def test_circuit_opens_after_threshold(self, storage: QdrantStorage, client: AsyncMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_circuit_open_blocks_requests(self, storage: QdrantStorage, client: AsyncMock) -> None:
        from backend.errors import CircuitOpenError
    ...
    @pytest.mark.asyncio
    async def test_circuit_half_open_after_cooldown(self, storage: QdrantStorage, client: AsyncMock) -> None:
...
class TestRetryBehavior:
    @pytest.mark.asyncio
    async def test_batch_upsert_retries_on_transient_error(self, storage: QdrantStorage, client: AsyncMock) -> None:
...
class TestDataClasses:
    ...
    def test_qdrant_point_without_sparse(self) -> None:
        point = QdrantPoint(id=5, vector=DENSE_VECTOR, sparse_vector=None, payload=VALID_PAYLOAD)
