...
class TestConversationStateSchema:
    ...
    def test_confidence_score_is_int(self):
        ...
        hints = get_type_hints(ConversationState)
        assert hints["confidence_score"] is int, f"Expected int, got {hints['confidence_score']}"
...
class TestResearchStateSchema:
    ...
    def test_confidence_score_is_float(self):
        ...
        hints = get_type_hints(ResearchState)
        assert hints["confidence_score"] is float, f"Expected float, got {hints['confidence_score']}"
...
class TestMetaReasoningStateSchema:
    ...
    def test_field_count(self):
        ...
        assert len(hints) == 11, f"Expected 11 fields, got {len(hints)}: {list(hints.keys())}"
...
class TestConversationGraphNodes:
    ...
    def test_fan_out_var_keyword(self):
        ...
        assert inspect.Parameter.VAR_KEYWORD in param_kinds, "fan_out must accept **kwargs"
    ...
    def test_aggregate_answers_var_keyword(self):
        ...
        param_kinds = [p.kind for p in sig.parameters.values()]
        assert inspect.Parameter.VAR_KEYWORD in param_kinds, "aggregate_answers must accept **kwargs"
    ...
    def test_summarize_history_var_keyword(self):
        ...
        param_kinds = [p.kind for p in sig.parameters.values()]
        assert inspect.Parameter.VAR_KEYWORD in param_kinds, "summarize_history must accept **kwargs"
    ...
    def test_format_response_var_keyword(self):
        ...
        param_kinds = [p.kind for p in sig.parameters.values()]
        assert inspect.Parameter.VAR_KEYWORD in param_kinds, "format_response must accept **kwargs"
    ...
    def test_handle_collection_mgmt_var_keyword(self):
        ...
        param_kinds = [p.kind for p in sig.parameters.values()]
        assert inspect.Parameter.VAR_KEYWORD in param_kinds, "handle_collection_mgmt must accept **kwargs"
    ...
    def test_request_clarification_no_di(self):
        ...
        has_kwargs = any(
            p.kind in (inspect.Parameter.VAR_KEYWORD, inspect.Parameter.KEYWORD_ONLY) for p in params.values()
        )
        assert not has_kwargs, "request_clarification should have no DI (no **kwargs or keyword-only params)"
...
class TestResearchGraphNodes:
    ...
    def _assert_has_config_param(self, func_name: str, func) -> None:
        ...
        assert config_param.default is None, f"{func_name} 'config' default must be None"
    ...
    def test_fallback_response_no_config(self):
        ...
        assert "config" not in params, "fallback_response must NOT have 'config' param"
    ...
    def test_all_nodes_exist(self):
        ...
        for name in ["orchestrator", "tools_node", "compress_context", "collect_answer", "fallback_response"]:
...
class TestMetaReasoningGraphNodes:
    ...
    def test_generate_alternative_queries_has_config(self):
        ...
        self._assert_has_config_param("generate_alternative_queries", generate_alternative_queries)
    ...
    def test_evaluate_retrieval_quality_has_config(self):
        ...
        self._assert_has_config_param("evaluate_retrieval_quality", evaluate_retrieval_quality)
...
class TestToolFactory:
    ...
    def test_create_research_tools_return_annotation(self):
        ...
        assert ret is list or ret == "list", f"Return annotation must be list, got {ret!r}"
...
class TestGraphBuilders:
    ...
    def test_all_builders_are_functions(self):
        ...
        for builder in [build_conversation_graph, build_research_graph, build_meta_reasoning_graph]:
