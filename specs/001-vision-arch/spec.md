...
- **FR-008a**: System MUST display a confidence indicator (0–100% scale) to the user alongside every answer in the main chat view, not only in the trace. A score of 100% indicates maximum confidence; 0% indicates minimum.
...
- **Answer**: A generated response to a query, including cited source passages and a confidence indicator (0–100% scale) displayed visibly to the user in the answer view. Confidence reflects the system's certainty that the answer is grounded in the retrieved documents.
...
- Q: What scale should the confidence indicator use? → A: 0–100% percentage scale (e.g., 87% confident). This makes SC-008 acceptance testing directly comparable: a "95% accuracy" target aligns with a 0–100% confidence threshold.
