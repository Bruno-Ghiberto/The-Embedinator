...
**Headline numbers (A1)**: warm-state factoid p50 ≈ **26,300 ms** vs SC-004 target 4,000 ms = **6.5× over budget**. GPU at 92–100% SM during generation — bottleneck is **token volume / round-trip count**, not hardware capacity. Per-stage telemetry (`stage_timings_json`) is **NULL in all traces** (FINDING COLD-004) — ranking below is architecture-derived from A2's call-graph audit and validated against A1's total-latency decomposition; A6 must add ad-hoc instrumentation to confirm before Iteration 1 fix.
...
- **Telemetry blocker (FINDING COLD-004)**: `stage_timings_json` is NULL in every trace. A6 should add ad-hoc instrumentation (timing prints around each stage) before Iteration 1 fix to validate this synthesis against measured per-stage contribution. A7 (Wave 4) writes the FR-008/SC-007 validation test — that test will FAIL until the recording site is repaired; A8 must surface this in `bug-registry-spec26.md`.
